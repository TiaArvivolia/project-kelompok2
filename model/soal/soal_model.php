...
class Soal
{
    ...
    {
        $result = $this->db->query("SELECT * FROM {$this->table} WHERE kategori_id = 1");
        return $result->fetch_all(MYSQLI_ASSOC); // Fetch as associative array
    }
    public function getSoalMahasiswa()
    {
        $result = $this->db->query("SELECT * FROM {$this->table} WHERE kategori_id = 2");
        return $result->fetch_all(MYSQLI_ASSOC); // Fetch as associative array
    }
    public function getSoalTendik()
    {
        $result = $this->db->query("SELECT * FROM {$this->table} WHERE kategori_id = 3");
        return $result->fetch_all(MYSQLI_ASSOC); // Fetch as associative array
    }
    public function getSoalAlumni()
    {
        $result = $this->db->query("SELECT * FROM {$this->table} WHERE kategori_id = 4");
        return $result->fetch_all(MYSQLI_ASSOC); // Fetch as associative array
    }
    public function getSoalOrtu()
    {
        $result = $this->db->query("SELECT * FROM {$this->table} WHERE kategori_id = 5");
        return $result->fetch_all(MYSQLI_ASSOC); // Fetch as associative array
    } 
